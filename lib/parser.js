...
function symbol(id, template, parse, transition) {
    var s = {};
    s.id = s.value = id;
    s.template = template || "<span/>"; 
    s.parse = parse || function() { return this };
...
// Generates a Y.Node from a token. String tokens are returned
// as plain strings. reST-specific information is stored on 
// the Y.Node using Y.Node.setData().
//
// Nodes need their own parse() and transition() functions. This should be stored with the symbol. 
