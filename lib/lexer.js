...
// Not yet supported:
// - Hyperlink references: only `foo bar`_ supported. Not 
function lex(input) {
    var i, tokens = [];
...
function process_line(line) {
    ...
    var token_line = [];
    var last_index = 0;
    // role pattern is bad, \w* is not correct.
...
function inline_token(matches) {
    ...
    function role_tokens() {
        ...
        var role_index   = (raw_tokens[0].length > 0) ? 0 : 1;
        var interp_index = (raw_tokens[0].length > 0) ? 1 : 0;
        var role_value = raw_tokens[role_index].replace(/:/g, "");
        
        var tokens = [2];
        tokens[role_index]   = {id: "(role)", value: role_value};
        tokens[interp_index] = {id: "`", value: "`"}
        return tokens;
    }
